...
type BridgeMasterConfig struct {
...
func CreateBridgeMaster(config *BridgeMasterConfig) error {
...
type BridgeSlaveConfig struct {
	InterfaceName string
	MasterName    string
}

func CreateBridgeSlave(config *BridgeSlaveConfig) error {
	connectionUUID, err := uuid.NewUUID()
	if err != nil {
		return err
	}
